...
void Dungeon::read_character() {
	character.read_characteristics("files\\Character.txt");
	// еще сохранять изменения надо, но не здесь
	character.read_skill_table
	("files\\Draining.txt",
		"files\\Necromancy.txt",
		"files\\Curse.txt");
}
void Dungeon::save_character() {
	character.save_characteristics("files\\Character_Saved.txt");
}
void Dungeon::read_saved_character() {
	character.read_characteristics("files\\Character_Saved.txt");
}
...
void Dungeon::read_levels() {
	...
	levels[0].read_squares("files\\Level_1_Squares.txt");
	levels[0].read_enemies("files\\Level_1_Enemies.txt");
	levels[1].read_squares("files\\Level_2_Squares.txt");
	levels[1].read_enemies("files\\Level_2_Enemies.txt");
	levels[2].read_squares("files\\Level_3_Squares.txt");
	levels[2].read_enemies("files\\Level_3_Enemies.txt");
}
void Dungeon::read_saved_levels() {
	levels[0].read_squares("files\\Level_1_Squares_Saved.txt");
	levels[0].read_enemies("files\\Level_1_Enemies_Saved.txt");
	levels[1].read_squares("files\\Level_2_Squares_Saved.txt");
	levels[1].read_enemies("files\\Level_2_Enemies_Saved.txt");
	levels[2].read_squares("files\\Level_3_Squares_Saved.txt");
	levels[2].read_enemies("files\\Level_3_Enemies_Saved.txt");
}
void Dungeon::save_levels() {
	levels[0].save_squares("files\\Level_1_Squares_Saved.txt");
	levels[0].save_enemies("files\\Level_1_Enemies_Saved.txt");
	levels[1].save_squares("files\\Level_2_Squares_Saved.txt");
	levels[1].save_enemies("files\\Level_2_Enemies_Saved.txt");
	levels[2].save_squares("files\\Level_3_Squares_Saved.txt");
	levels[2].save_squares("files\\Level_3_Enemies_Saved.txt");
}
...
void Dungeon::go_enemies(int k) {
	std::vector <char> ways;
	ways = { 'w','a','s','d' };
	std::vector<Enemy*> enemies = levels[k].get_enemies();
	for (int tmp = 0; tmp < enemies.size(); tmp++) {
		int i = 0;
		i = rand() % 4;
		if (ways[i] == 'w') enemies[tmp]->set_y(enemies[tmp]->get_y() + 1);

		if (ways[i] == 's') enemies[tmp]->set_y(enemies[tmp]->get_y() - 1);
		if (ways[i] == 'a') enemies[tmp]->set_x(enemies[tmp]->get_x() - 1);
	
		if (ways[i] == 'd')	enemies[tmp]->set_x(enemies[tmp]->get_x() + 1);
	}
}